<template>
  <v-container id="odd-even" class="pt-0">
    <v-row>
...
export default {
  ...
  data() {
    return {
      ...
      barChartRef: "oddEvenBarChart",
      barChartXAxisName: "奇偶",
      barChartXAxisData: [],
      barChartYAxisName: "个数",
      barChartData: [],

      pieChartRef: "oddEvenPieChart",
      pieChartLegendData: [],
      pieChartData: []
    };
  },
  ...
  methods: {
    ...
    lastestChanged() {
      ...
      for (let i = 0; i < this.lastest; i++) {
        ...
        let numAttr = "";
        for (let char of item) numAttr += Number(char) % 2 == 0 ? "偶" : "奇";
        oddEvenCounter[numAttr]++;
        winningOddEven[item] = numAttr;
      }

      /* 生成表格数据 */
      let oddEven = Object.keys(oddEvenCounter);
      this.vuetifyTableData.headers = [];
      for (let i of oddEven) {
      ...
      for (let i = 0; i < this.lastest; i++) {
        ...
        let item = {};
        for (let j of oddEven) item[j] = "";
        item[winningOddEven[winningNo]] = winningNo;
        this.vuetifyTableData.items.push(item);
      }

      /* bar chart 数据 */
      this.barChartXAxisData = oddEven;
      this.barChartData = Object.values(oddEvenCounter);

      /* pie chart 数据 */
      this.pieChartData = [];
      for (let i in oddEvenCounter) {
        let item = {};
        item["name"] = i;
        item["value"] = oddEvenCounter[i];
        this.pieChartData.push(item);
      }
      this.pieChartLegendData = oddEven;
    }
  }
};
</script>
