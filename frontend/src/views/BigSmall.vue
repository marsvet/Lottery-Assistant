<template>
  <v-container id="big-small" class="pt-0">
    <v-row>
...
export default {
  ...
  data() {
    return {
      ...
      barChartRef: "bigSmallBarChart",
      barChartXAxisName: "大小",
      barChartXAxisData: [],
      barChartYAxisName: "个数",
      barChartData: [],

      pieChartRef: "bigSmallPieChart",
      pieChartLegendData: [],
      pieChartData: []
    };
  },
  ...
  methods: {
    ...
    lastestChanged() {
      ...
      for (let i = 0; i < this.lastest; i++) {
        ...
        let numAttr = "";
        for (let char of item) numAttr += Number(char) <= 4 ? "小" : "大";
        bigSmallCounter[numAttr]++;
        winningBigSmall[item] = numAttr;
      }

      /* 生成表格数据 */
      let bigSmall = Object.keys(bigSmallCounter);
      this.vuetifyTableData.headers = [];
      for (let i of bigSmall) {
      ...
      for (let i = 0; i < this.lastest; i++) {
        ...
        let item = {};
        for (let j of bigSmall) item[j] = "";
        item[winningBigSmall[winningNo]] = winningNo;
        this.vuetifyTableData.items.push(item);
      }

      /* bar chart 数据 */
      this.barChartXAxisData = bigSmall;
      this.barChartData = Object.values(bigSmallCounter);

      /* pie chart 数据 */
      this.pieChartData = [];
      for (let i in bigSmallCounter) {
        let item = {};
        item["name"] = i;
        item["value"] = bigSmallCounter[i];
        this.pieChartData.push(item);
      }
      this.pieChartLegendData = bigSmall;
    }
  }
};
</script>
